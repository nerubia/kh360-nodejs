 ...
 * @param req.query.payment_reference_no - Filter by payment reference no.
 * @param req.query.status - Filter by status.
 * @param req.query.page - Page number for pagination.
 */
export const index = async (req: Request, res: Response) => {
  try {
    const { payment_date, client_id, invoice_no, payment_reference_no, status, page } = req.query
    const results = await PaymentService.getAllByFilters(
      payment_date as string,
      parseInt(client_id as string),
      invoice_no as string,
      payment_reference_no as string,
      status as string,
      page as string
    )
    res.json(results)
